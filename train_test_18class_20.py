...
class TwoBranchNet(nn.Module):
    def __init__(self, pad_len, imu_dim, tof_dim, n_classes):
        ...
        self.imu_branch = nn.Sequential(
            ResidualSEBlock(imu_dim, 128, kernel_size=3, dropout=0.1),
            ResidualSEBlock(128, 256, kernel_size=5, dropout=0.1)
        )

        self.tof_branch = nn.Sequential(
            nn.Conv1d(tof_dim, 128, 3, padding=1, bias=False),
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.MaxPool1d(2),
            nn.Dropout(0.2),
        ...
        self.bi_lstm = nn.LSTM(input_size=512, hidden_size=256, batch_first=True, bidirectional=True)
        self.bi_gru = nn.GRU(input_size=512, hidden_size=256, batch_first=True, bidirectional=True)
        ...
        self.attention = Attention(256 * 4 + 64)

        self.classifier = nn.Sequential(
            nn.Linear(256 * 4 + 64, 512, bias=False),
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Dropout(0.5),
...
# 学習・評価・保存処理は前回のまま適用可能。必要に応じてその部分もここに統合可能です。
# ========= Training Config & Preprocessing ==========
RAW_CSV = "train.csv"
SAVE_DIR = "train_test_18class_20"
PAD_PERCENTILE = 95
BATCH_SIZE = 64
EPOCHS = 160
MIXUP_ALPHA = 0.4
