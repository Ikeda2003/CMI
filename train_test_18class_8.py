...
for epoch in range(EPOCHS):
    ...
    print(f"Epoch {epoch+1:2d} | Loss: {total_loss:.4f} | Acc18: {acc_18:.4f}, F1_18: {f1_18:.4f} | "
          f"Acc_bin: {acc_bin:.4f}, F1_bin: {f1_bin:.4f} | Acc_9: {acc_9:.4f}, F1_9: {f1_9:.4f} | F1_avg_2+9: {f1_avg:.4f}")

    if (epoch + 1) % 20 == 0:
        torch.save(model.state_dict(), f"multimodal_fusion_model_epoch{epoch+1}.pt")
        print(f"\U0001F4E6 モデルを保存しました → epoch {epoch+1}")

torch.save(model.state_dict(), "multimodal_fusion_model_final.pt")
print("\u2705 最終モデル保存完了")
