...
class MultimodalModel_TransformerFusion(nn.Module):
    def __init__(self, imu_dim, num_classes=18, d_model=512, nhead=8, num_layers=2):
        super().__init__()
        ...
        self.fusion_att = Attention(d_model)
    ...
    def forward(self, imu_x, tof_x):
        ...
        fused = torch.cat([imu_feat, x_tof], dim=2)  # (B, T, 512)
        x = self.transformer_encoder(fused)
        x = self.fusion_att(x)
...
BATCH_SIZE = 128
EPOCHS = 50
SAVE_DIR = "train_test_18class_15"
...
for fold, (tr_idx, va_idx) in enumerate(kf.split(all_seq, groups=subjects)):
    ...
    model = MultimodalModel_TransformerFusion(imu_dim=len(main_cols), num_classes=num_classes).to(device)
